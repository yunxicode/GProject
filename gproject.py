...
Original file is located at
    https://colab.research.google.com/drive/1S-jzMDBbxGK3N9-VE8aoss9Z_ymgGPcu
"""
...
matplotlib.font_manager.fontManager.addfont('C:\\OneDrive\\桌面\\GProject\\SimHei.ttf') # 呈現中文
...
pdfmetrics.registerFont(TTFont('MSJH', '/content/drive/My Drive/NProject/fonts/MSJH.TTC'))
pdfmetrics.registerFont(TTFont('MSJHBD', '/content/drive/My Drive/NProject/fonts/MSJHBD.TTC'))
...
jieba.set_dictionary("/content/drive/My Drive/NProject/dict.txt")
# 載入自定義詞庫：jieba.load_userdict(file_path)
# 加入字詞：jieba.add_word(word, freq=None, tag=None)
# 刪除字詞：jieba.del_word(word)

with open('/content/drive/My Drive/NProject/stopwords_zhTW.txt') as f:
...
def load_file():
    file1 = ''
    with open('/content/drive/My Drive/GProject/uploads/TEXT1.txt','r', encoding = 'utf-8') as load:
        file1 = load.readlines()
        load.close()
    file2 = ''
    with open('/content/drive/My Drive/GProject/uploads/TEXT2.txt', 'r', encoding = 'utf-8') as mem:
        file2 = mem.readlines()
    mem.close()
    return file1, file2
...
def img(all_part):
    ...
    raw = open("/content/drive/My Drive/NProject/TEXT1.txt").read() 
    text1 = nltk.text.Text(jieba.cut(raw))
    graph1 = nltk.text.Text(text1)
    graph1.dispersion_plot(all_part)
    fig.savefig('/content/drive/My Drive/GProject/output_files/plot.png')
...
UPLOAD_FOLDER = '/content/drive/My Drive/GProject/uploads'
ALLOWED_EXTENSIONS = set(['txt', 'pdf'])
app = Flask(__name__, template_folder='/content/drive/My Drive/GProject/templates', static_folder='/content/drive/My Drive/GProject/static', static_url_path='/content/drive/My Drive/GProject/static')
...
def background(): 
    return send_from_directory(os.path.join(app.root_path, '/content/drive/My Drive/GProject/static'), 'Background.png')
@app.route('/favicon.svg')
def favicon(): 
    return send_from_directory(os.path.join(app.root_path, '/content/drive/My Drive/GProject/static'), 'favicon.svg')
...
def compared_file():
    ...
    photo = Image('/content/drive/My Drive/GProject/output_files/plot.png', width=376, height=273)
    ...
    score = [['文本1', '文本2']]
    score_ = score1.append(score2)
    score = score.append(score_)
    score_F = Table(score, colWidths=240, style=table_style)
